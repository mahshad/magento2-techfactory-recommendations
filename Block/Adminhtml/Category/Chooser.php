...
/**
 * Prepare category chooser html
 */
namespace Techfactory\Recommendations\Block\Adminhtml\Category;
...
class Chooser extends Template
{
    ...
    public function prepareElementHtml(AbstractElement $element)
    {
        ...
        $data['style'] = 'width: auto !important;';
        $htmlItem = $this->_elementFactory->create('text', ['data' => $data]);
        $htmlItem
                ->setId("{$htmlId}")
                ->setForm($element->getForm())
                ->addClass('input-text admin__control-text element-value-changer readonly')
                ->addClass('entities')
    ...
    protected function _afterElementHtml($element)
    {
        ...
        $return = <<<HTML
            <a href="javascript:void(0)"
                onclick="MultiCategoryChooser.displayChooser('{$htmlId}-container')"
                class="widget-option-chooser"
                title="Open Chooser">
                <img src="{$this->getViewFileUrl('images/rule_chooser_trigger.gif')}" alt="Open Chooser" />
            </a>
            <a href="javascript:void(0)"
                onclick="MultiCategoryChooser.hideChooser('{$htmlId}-container')"
                title="Apply">
